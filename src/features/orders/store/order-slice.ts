...
export type OrderSlice = {
  ...
  addOrder: (request: CreateOrderRequest) => Promise<void>
  editOrder: (id: string, request: UpdateOrderRequest) => Promise<void>
...
export const createOrderSlice: StateCreator<OrderSlice, [], [], OrderSlice> = (set, get) => ({
  ...
  addOrder: async (request) => {
    ...
    try {
      const real = await orderService.createOrder(request)
      set((s) => ({
  ...
  editOrder: async (id, request) => {
    ...
    const optimistic: Order = { ...prev, ...request, updatedAt: new Date().toISOString() }
    ...
    try {
      const real = await orderService.updateOrder(prev, request)
      set((s) => ({
